...
- 场外基金详情新增后台快照极速模式：当前基金已存在 `otc_watch_snapshot` 时，详情页直接读取快照评分和本地净值缓存，跳过前台慢速实时接口。
- 场外短线穿透新增“实时穿透净值/实时穿透涨幅”：当全A实时行情接口失败时，会按基金前20重仓股单独抓取实时行情并估算当日净值变化。
- `净值与动量` 会把实时穿透净值追加为最新净值点，用于当天 DIF、DEA、MACD 柱和短线动量观察。
- 云端 PostgreSQL 模式默认启用“云端极速读库模式”：网页端只读数据库快照，场外净值历史从 `otc_nav_history` 表读取；爬取和刷新交给 GitHub Actions 后台，避免手机打开页面时等待实时接口。
...
云端工作流默认在 A 股交易时段约每 10 分钟刷新一次。场外后台采集会写入两类表：

- `otc_watch_snapshot`：场外自选基金短线评分排行、实时穿透涨幅、动作提示等。
- `otc_nav_history`：每只场外基金最近一次净值历史，用于前台快速绘制净值、均线、DIF/DEA/MACD。
